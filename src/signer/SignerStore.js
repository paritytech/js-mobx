...
export default class SignerStore {
  ...
  fetchBalances (_addresses) {
    ...
    Promise.all(addresses.map(address => this._api.eth.getBalance(address)))
      .then(_balances => {
        this.setBalances(
  ...
  fetchLocalTransactions = () => {
    ...
    this._api.parity
      ...
      .then(localTransactions => {
        const keys = Object.keys(localTransactions).filter(
          key => localTransactions[key].status !== 'canceled'
        );
