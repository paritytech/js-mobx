...
const methodGroups = {
  ...
  accountsDelete: {
    methods: ['parity_killAccount', 'parity_removeAddress']
  },
  vaults: {
    methods: [
      'parity_closeVault',
  ...
  signerRequests: {
    methods: ['parity_checkRequest', 'parity_localTransactions']
  },
  signerConfirm: {
    methods: [
  ...
  nodeUpgrade: {
    methods: ['parity_executeUpgrade']
  }
};

/**
 * Get a map of method->methodGroup
 */
const methodGroupFromMethod = {};
 ...
 */
const allMethods = flatten(
  Object.keys(methodGroups).map(key => methodGroups[key].methods)
);

export { allMethods, methodGroupFromMethod };
